...
do
        (( yearm1 = year - 1 ))
        if [ "$(ls cdas1.${yearm1}1231.pgrbh.grb2.nc.bz2 cdas1.$year????.pgrbh.grb2.nc.bz2 2>/dev/null)" ] 
        then
                for file in cdas1.${yearm1}1231.pgrbh.grb2.nc.bz2 cdas1.$year????.pgrbh.grb2.nc.bz2
                do
                        [[ ! -f $file ]] && continue
                        bunzip2 $file &
                done
        ...
        for file in cdas1.${yearm1}1231.pgrbh.grb2.nc cdas1.$year????.pgrbh.grb2.nc
        do
                [[ ! -f $file ]] && continue
                no=$(echo $file | cut -f2 -d.)
